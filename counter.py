...
while True:
    ...
    temp_list = [[frame] + item for item in lmList] ##make list shape ([[frame=1,joint=1,x,y],[frame=1,joint=2,x,y],[...]] scope=inside while loop
...
#     if len(vel_x) > 1:
#         rft = np.fft.rfft(vel_x)
#         rft[5:] = 1  # Note, rft.shape = 21
#         x_smooth = np.fft.irfft(rft)
#         ax_smooth = np.diff(x_smooth)
#         #    #idx = np.argwhere(np.diff(np.sign(ax_smooth))).flatten() #find intersections of ax_smooth with x axis
        #print(idx)
...
#print(lm_df) #around 1000 rows per second, 30 frames x 33 joints
print(lm_df.to_markdown())
#plt.plot(lm_df)
#plt.plot(list_of_frames, vel_x, marker='o', linestyle='-')
#plt.plot(list_of_frames[:len(acc_x)], acc_x, marker='o', linestyle='-')
#plt.plot(list_of_frames[:len(x_smooth)], x_smooth, marker='o', linestyle='-')
#plt.plot(list_of_frames[:len(ax_smooth)], ax_smooth, marker='o', linestyle='-')
#plt.show()