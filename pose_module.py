...
class poseDetector():
    ...
    def __init__(self,
        ...
        self.seg_smooth = smooth_segmentation
        ...
        self.pose = self.mpPose.Pose(self.mode, self.complexity, self.smooth, self.segmentation,
                                     self.seg_smooth, self.detectionCon, self.trackCon)
...
def main():
    ...
    while True:
        ...
        img = detector.findPose(img)  # add False to turn off skeleton
        lmList = detector.findPosition(img, draw=False)
        if len(lmList) != 0:
            angle = detector.findAngle(img, 11, 13, 15) #get left elbow angle
